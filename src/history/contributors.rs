...
pub fn rank(
    ...
    let mut findings: Vec<HistoryFinding> = per_file
        .into_iter()
        .filter_map(|(file, counts)| build_fragmentation(file, counts, t))
        .collect();
...
fn build_fragmentation(
    ...
    let minors: Vec<&(String, u32)> = author_counts
        .iter()
        .filter(|(_, count)| f64::from(*count) / total_f < pct_threshold)
        .collect();
    let minor_count = u32::try_from(minors.len()).unwrap_or(u32::MAX);
    if minor_count < t.contributors.min_minor_authors {
        return None;
    }
    let top_minor_authors: Vec<String> =
        minors.iter().take(5).map(|(a, _)| a.clone()).collect();
    let total_contributors = u32::try_from(total_authors).unwrap_or(u32::MAX);
    let observed_pct = if total_contributors == 0 {
        0.0
    } else {
        f64::from(minor_count) / f64::from(total_contributors)
    };
    Some(HistoryFinding {
...
fn sort_fragmentation_findings(findings: &mut [HistoryFinding]) {
    findings.sort_by(|a, b| {
        ...
        eb.minor_contributor_count
            .cmp(&ea.minor_contributor_count)
