...
use super::{
    compute_assert_fingerprint, compute_structural_fingerprint, count_code_lines,
    count_consecutive_asserts, find_child_by_kind, measure_nesting_depth, node_text, FileMetrics,
...
fn analyze_function(node: Node, source: &str) -> Option<FunctionMetrics> {
    let declarator = find_child_by_kind(node, "function_declarator")
        .or_else(|| find_child_by_kind(node, "pointer_declarator").and_then(|p| find_child_by_kind(p, "function_declarator")))?;
