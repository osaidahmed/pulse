...
pub fn count_string_match_arms(
    ...
    walk_for_string_matches(body, match_kind, case_kind, string_kinds, &mut max_arms);
    max_arms
}

fn walk_for_string_matches(
    node: Node,
    match_kind: &str,
    case_kind: &str,
    string_kinds: &[&str],
    ...
    while let Some(child) = child_opt {
        if child.kind() == match_kind {
            let arms = tally_string_cases(child, case_kind, string_kinds);
            if arms > *max_arms { *max_arms = arms; }
        } else {
            walk_for_string_matches(child, match_kind, case_kind, string_kinds, max_arms);
        }
