...
const BOOL_OPS: &[&str] = &["&&", "||"];
const BOOL_STOPS: &[&str] = &["block", "method_declaration", "class_declaration", "closure", "lambda_expression"];
...
const LOOP_KINDS: &[&str] = &["for_statement", "enhanced_for_statement", "while_statement", "do_statement"];
...
const FUNC_DEF_CFG: CallableConfig = CallableConfig { body_kind: "closure", fallback_name: "<anonymous>" };
...
fn walk_groovy_consequence(node: Node, source: &str, depth: u32, saw_else: bool, s: &mut WalkState) {
    ...
    let target = if node.kind() == "expression_statement" {
        find_child_by_kind(node, "closure").or_else(|| find_child_by_kind(node, "block")).unwrap_or(node)
    } else {
        node
    };
...
fn count_declarations(root: Node) -> u32 {
    ...
    root.children(&mut cursor)
        .filter(|c| matches!(c.kind(), "class_declaration" | "interface_declaration" | "enum_declaration"))
        .count() as u32
}
