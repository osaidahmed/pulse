...
pub fn keep_expression_clusters(clusters: Vec<RawCluster>, kinds_by_fp: &KindIndex) -> Vec<RawCluster> {
    clusters.into_iter().filter(|c| is_expression_level(c.fingerprint, kinds_by_fp)).collect()
}
