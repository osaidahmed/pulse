...
fn rust_large_struct_detected() {
    ...
    for i in 0..15 { code.push_str(&format!("    f{i}: i32,\n")); }
    code.push_str("}\n");
    let out = check(&code, "rs");
    assert!(has_smell(&out, "Large Struct"), "15 fields should trigger, got: {}", out);
}
...
fn rust_struct_at_threshold_not_flagged() {
    ...
    for i in 0..12 { code.push_str(&format!("    f{i}: i32,\n")); }
    code.push_str("}\n");
    let out = check(&code, "rs");
    assert!(!has_smell(&out, "Large Struct"), "12 fields = threshold, should NOT flag");
}
...
fn python_short_vars_detected() {
    ...
    for i in 0..10 { code.push_str(&format!("    var{i} = {i}\n")); }
...
fn python_short_vars_exempt_ijk() {
    let mut code = String::from("def func():\n    i = 0\n    j = 0\n    k = 0\n");
    for n in 0..15 { code.push_str(&format!("    x{n} = {n}\n")); }
    code.push_str("    return 0\n");
...
fn python_short_vars_count_in_debug() {
    ...
    for i in 0..12 { code.push_str(&format!("    x{i} = {i}\n")); }
    code.push_str("    return 0\n");
    let out = dbg(&code, "py");
...
fn rust_short_vars_detected() {
    ...
    for i in 0..10 { code.push_str(&format!("    let v{i} = {i};\n")); }
    code.push_str("    0\n}\n");
...
fn rust_short_vars_exempt_loop_counter() {
    let mut code = String::from("fn func() -> i32 {\n    let i = 0;\n    let j = 0;\n    let k = 0;\n");
    for n in 0..15 { code.push_str(&format!("    let v{n} = {n};\n")); }
    code.push_str("    0\n}\n");
    assert!(!has_smell(&check(&code, "rs"), "Short Variable Names"));
}
...
fn go_short_vars_detected() {
    ...
    for c in "abcdefgh".chars() { code.push_str(&format!("    {c} := 1\n")); }
    for i in 0..10 { code.push_str(&format!("    v{i} := {i}\n")); }
    code.push_str("    return 0\n}\n");
...
fn java_short_vars_detected() {
    ...
    for i in 0..10 { code.push_str(&format!("        int v{i} = {i};\n")); }
    code.push_str("        return 0;\n    }\n}\n");
...
fn large_struct_shows_threshold() {
    ...
    for i in 0..15 { code.push_str(&format!("    f{i}: i32,\n")); }
    code.push_str("}\n");
    assert!(check(&code, "rs").contains("threshold: 12"));
}
...
fn short_vars_shows_threshold() {
    let mut code = String::from("def func():\n");
    for c in "abcdefgh".chars() { code.push_str(&format!("    {c} = 1\n")); }
    for i in 0..10 { code.push_str(&format!("    x{i} = {i}\n")); }
    code.push_str("    return 0\n");
    assert!(check(&code, "py").contains("threshold: 3"));
}
...
fn stringly_typed_shows_threshold() {
    ...
    assert!(check(code, "rs").contains("threshold: 5"));
}
...
fn struct_at_13_fields_flagged() {
    let mut code = String::from("struct S {\n");
    for i in 0..13 { code.push_str(&format!("    f{i}: i32,\n")); }
    code.push_str("}\n");
    assert!(has_smell(&check(&code, "rs"), "Large Struct"));
}
...
fn short_vars_at_3_not_flagged() {
    let mut code = String::from("def func():\n    a = 1\n    b = 2\n    c = 3\n");
    for i in 0..15 { code.push_str(&format!("    v{i} = {i}\n")); }
    code.push_str("    return 0\n");
