use pulse::audit::categorize::categorize;
use pulse::audit::finding::PatternCategory;
...
fn categorize_literal_root_yields_literal_repetition() {
    assert_eq!(
        categorize(&kinds(&["string", "identifier"])),
        PatternCategory::LiteralRepetition
    );
    assert_eq!(
        categorize(&kinds(&["integer"])),
        PatternCategory::LiteralRepetition
    );
}
...
fn categorize_call_root_yields_method_call() {
    assert_eq!(
        categorize(&kinds(&["call", "identifier"])),
        PatternCategory::MethodCall
    );
    assert_eq!(
        categorize(&kinds(&["call_expression", "identifier"])),
        PatternCategory::MethodCall
    );
}
...
fn categorize_dict_root_yields_dict_literal() {
    assert_eq!(
        categorize(&kinds(&["dictionary", "pair"])),
