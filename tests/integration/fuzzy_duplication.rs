...
fn very_different_sizes_no_match() {
    ...
    for i in 0..30 {
        code.push_str(&format!("    x{i} = process(data, {i})\n"));
    }
...
fn similar_functions_in_typescript() {
    let mut code =
        String::from("function processA(data: any): any {\n    const result: any = {};\n    if (!data) return null;\n");
    for i in 0..17 {
        code.push_str(&format!("    result.f{i} = transform_a(data, {i});\n"));
    }
    code.push_str("    return result;\n}\n\n");
    code.push_str("function processB(info: any): any {\n    const output: any = {};\n    if (!info) return null;\n");
    for i in 0..17 {
        code.push_str(&format!("    output.f{i} = transform_b(info, {i});\n"));
    }
...
fn similar_functions_in_rust() {
    ...
    for i in 0..18 {
        code.push_str(&format!("    let s{i} = data[{i}];\n"));
    }
    code.push_str("    s0\n}\n\n");
    code.push_str("fn process_b(input: &[i32]) -> i32 {\n    if input.is_empty() { return 0; }\n");
    for i in 0..18 {
        code.push_str(&format!("    let v{i} = input[{i}];\n"));
    }
    code.push_str("    v0\n}\n");
...
fn similar_functions_in_java() {
    ...
    for i in 0..17 {
        code.push_str(&format!("        int s{i} = d[{i}];\n"));
    }
    code.push_str("        return s0;\n    }\n\n    int processB(int[] e) {\n        if (e == null) return 0;\n");
    for i in 0..17 {
        code.push_str(&format!("        int v{i} = e[{i}];\n"));
    }
...
fn find_fn<'a>(fns: &'a [pulse::walk::FunctionMetrics], name: &str) -> &'a pulse::walk::FunctionMetrics {
    fns.iter().find(|f| f.name == name).unwrap_or_else(|| {
        panic!("expected function {name}, got {:?}", fns.iter().map(|f| &f.name).collect::<Vec<_>>())
    })
}
