...
lang_walker_smoke!(python_smoke_basic_function, Language::Python, "def f(x):\n    if x == 1:\n        return x\n    return 0\n");
lang_walker_smoke!(python_smoke_class, Language::Python, "class C:\n    def m(self):\n        return 1\n    def n(self):\n        return 2\n");
lang_walker_smoke!(python_smoke_async_function, Language::Python, "async def f(x):\n    if x == 1:\n        return await g(x)\n    return await g(0)\n");
lang_walker_smoke!(python_smoke_decorator, Language::Python, "@deco\ndef f(x):\n    if x == 1:\n        return x\n    return 0\n");
lang_walker_smoke!(python_smoke_match, Language::Python, "def f(x):\n    match x:\n        case 1: return 1\n        case _: return 0\n");
...
lang_walker_smoke!(python_smoke_with_statement, Language::Python, "with open(p) as f:\n    for line in f:\n        process(line)\n");
lang_walker_smoke!(python_smoke_try_except, Language::Python, "try:\n    do()\nexcept ValueError as e:\n    handle(e)\nexcept Exception:\n    fail()\n");
lang_walker_smoke!(python_smoke_typed_function, Language::Python, "def f(x: int, y: str = \"hi\") -> bool:\n    return x > 0 and y == \"hi\"\n");
lang_walker_smoke!(python_smoke_class_with_methods, Language::Python, "class Foo:\n    def __init__(self, x):\n        self.x = x\n    def get(self):\n        return self.x\n    def set(self, v):\n        self.x = v\n");
lang_walker_smoke!(python_smoke_nested_function, Language::Python, "def outer(x):\n    def inner(y):\n        return y * 2\n    return inner(x) + 1\n");
lang_walker_smoke!(python_smoke_class_with_property, Language::Python, "class C:\n    @property\n    def x(self):\n        return self._x\n    @x.setter\n    def x(self, v):\n        self._x = v\n");
lang_walker_smoke!(python_smoke_dataclass_pattern, Language::Python, "@dataclass\nclass Point:\n    x: int\n    y: int\n    label: str = \"\"\n");

lang_walker_smoke!(typescript_smoke_function, Language::TypeScript, "function f(x: number): number { if (x === 1) return x; return 0; }\n");
lang_walker_smoke!(typescript_smoke_arrow, Language::TypeScript, "const f = (x: number): number => x === 1 ? x : 0;\n");
lang_walker_smoke!(typescript_smoke_class, Language::TypeScript, "class A { constructor(public x: number) {} get value(): number { return this.x; } }\n");
lang_walker_smoke!(typescript_smoke_interface, Language::TypeScript, "interface I { name: string; getId(): number; }\n");
lang_walker_smoke!(typescript_smoke_generic, Language::TypeScript, "function id<T>(x: T): T { return x; }\n");
lang_walker_smoke!(typescript_smoke_async, Language::TypeScript, "async function fetchData(): Promise<number> { return await load(); }\n");
...
lang_walker_smoke!(typescript_smoke_template_literal, Language::TypeScript, "const greeting = `Hello, ${name}! You have ${count} messages.`;\n");
lang_walker_smoke!(typescript_smoke_enum, Language::TypeScript, "enum Color { Red = 1, Green = 2, Blue = 3 }\n");
lang_walker_smoke!(typescript_smoke_union_type, Language::TypeScript, "type Shape = { kind: \"circle\"; r: number } | { kind: \"square\"; s: number };\n");
lang_walker_smoke!(typescript_smoke_namespace, Language::TypeScript, "namespace utils { export function help() { return 1; } }\n");

lang_walker_smoke!(javascript_smoke_function, Language::JavaScript, "function f(x) { if (x === 1) return x; return 0; }\n");
lang_walker_smoke!(javascript_smoke_arrow, Language::JavaScript, "const f = x => x === 1 ? x : 0;\n");
lang_walker_smoke!(javascript_smoke_class, Language::JavaScript, "class A { constructor(x) { this.x = x; } get value() { return this.x; } }\n");
lang_walker_smoke!(javascript_smoke_async, Language::JavaScript, "async function load() { const data = await fetch(url); return data.json(); }\n");
...
lang_walker_smoke!(javascript_smoke_generator, Language::JavaScript, "function* gen() { yield 1; yield 2; yield 3; }\n");
...
lang_walker_smoke!(rust_smoke_enum, Language::Rust, "enum Shape { Circle(f64), Square { side: f64 }, Triangle(f64, f64, f64) }\n");
lang_walker_smoke!(rust_smoke_trait, Language::Rust, "trait Display { fn fmt(&self) -> String; fn pretty(&self) -> String { self.fmt() } }\n");
lang_walker_smoke!(rust_smoke_impl, Language::Rust, "impl Foo { fn new() -> Self { Self { x: 0 } } fn get(&self) -> i32 { self.x } }\n");
lang_walker_smoke!(rust_smoke_generic, Language::Rust, "fn id<T: Clone>(x: T) -> T { x.clone() }\n");
lang_walker_smoke!(rust_smoke_lifetime, Language::Rust, "fn longer<'a>(x: &'a str, y: &'a str) -> &'a str { if x.len() > y.len() { x } else { y } }\n");
lang_walker_smoke!(rust_smoke_async, Language::Rust, "async fn load() -> Result<Data, Error> { let d = fetch().await?; Ok(d) }\n");
lang_walker_smoke!(rust_smoke_macro_call, Language::Rust, "fn main() { let xs = vec![1, 2, 3]; println!(\"{:?}\", xs); }\n");
lang_walker_smoke!(rust_smoke_closure, Language::Rust, "fn f() { let add = |a, b| a + b; add(1, 2); }\n");
lang_walker_smoke!(rust_smoke_pattern_destructure, Language::Rust, "fn f(p: Point) { let Point { x, y } = p; let _ = x + y; }\n");

lang_walker_smoke!(go_smoke_function, Language::Go, "package p\nfunc f(x int) int { if x == 1 { return x }; return 0 }\n");
lang_walker_smoke!(go_smoke_struct, Language::Go, "package p\ntype Point struct { X int; Y int; Label string }\n");
lang_walker_smoke!(go_smoke_interface, Language::Go, "package p\ntype Reader interface { Read(p []byte) (int, error) }\n");
lang_walker_smoke!(go_smoke_method, Language::Go, "package p\ntype P struct { X int }\nfunc (p P) Get() int { return p.X }\n");
lang_walker_smoke!(go_smoke_goroutine, Language::Go, "package p\nfunc main() { go work(); ch := make(chan int); ch <- 1 }\n");
...
lang_walker_smoke!(go_smoke_select, Language::Go, "package p\nfunc f() { select { case x := <-ch1: use(x); case <-ch2: stop() } }\n");
lang_walker_smoke!(go_smoke_type_assertion, Language::Go, "package p\nfunc f(x interface{}) { if v, ok := x.(int); ok { use(v) } }\n");
lang_walker_smoke!(go_smoke_type_switch, Language::Go, "package p\nfunc f(x interface{}) { switch v := x.(type) { case int: use(v); default: skip() } }\n");

lang_walker_smoke!(java_smoke_class, Language::Java, "class A { int x; A(int x) { this.x = x; } int get() { return this.x; } }\n");
lang_walker_smoke!(java_smoke_method, Language::Java, "class A { int compute(int x) { if (x == 1) return x; return 0; } }\n");
lang_walker_smoke!(java_smoke_interface, Language::Java, "interface I { int compute(int x); default int doubled(int x) { return compute(x) * 2; } }\n");
lang_walker_smoke!(java_smoke_generic, Language::Java, "class Box<T> { T value; Box(T v) { this.value = v; } T get() { return value; } }\n");
lang_walker_smoke!(java_smoke_lambda, Language::Java, "class A { Function<Integer, Integer> f = x -> x + 1; }\n");
lang_walker_smoke!(java_smoke_stream, Language::Java, "class A { void f(List<Integer> xs) { xs.stream().filter(x -> x > 0).forEach(System.out::println); } }\n");
lang_walker_smoke!(java_smoke_anonymous_class, Language::Java, "class A { Runnable r = new Runnable() { public void run() { doWork(); } }; }\n");
lang_walker_smoke!(java_smoke_enum, Language::Java, "enum Color { RED, GREEN, BLUE; public int rank() { return ordinal(); } }\n");
lang_walker_smoke!(java_smoke_try_with_resources, Language::Java, "class A { void f() { try (Reader r = open()) { r.read(); } catch (Exception e) {} } }\n");
lang_walker_smoke!(java_smoke_record, Language::Java, "record Point(int x, int y) {}\n");

lang_walker_smoke!(csharp_smoke_class, Language::CSharp, "class A { int X { get; set; } public A(int x) { X = x; } }\n");
lang_walker_smoke!(csharp_smoke_method, Language::CSharp, "class A { int F(int x) { if (x == 1) return x; return 0; } }\n");
lang_walker_smoke!(csharp_smoke_property, Language::CSharp, "class A { public int X { get { return _x; } set { _x = value; } } }\n");
lang_walker_smoke!(csharp_smoke_lambda, Language::CSharp, "class A { Func<int, int> f = x => x + 1; }\n");
lang_walker_smoke!(csharp_smoke_linq, Language::CSharp, "class A { void F() { var xs = items.Where(x => x > 0).Select(x => x * 2).ToList(); } }\n");
lang_walker_smoke!(csharp_smoke_async, Language::CSharp, "class A { public async Task<int> F() { var r = await Load(); return r; } }\n");
lang_walker_smoke!(csharp_smoke_generic, Language::CSharp, "class Box<T> { public T Value { get; set; } public Box(T v) { Value = v; } }\n");
...
lang_walker_smoke!(c_smoke_for_loop, Language::C, "void f(int *xs, int n) { for (int i = 0; i < n; i++) { xs[i] = i * 2; } }\n");
lang_walker_smoke!(c_smoke_typedef, Language::C, "typedef struct { int x; int y; } Point;\n");
lang_walker_smoke!(c_smoke_function_pointer, Language::C, "int (*fp)(int) = f; int call(int (*g)(int), int x) { return g(x); }\n");
lang_walker_smoke!(c_smoke_macro, Language::C, "#define MAX(a, b) ((a) > (b) ? (a) : (b))\nint biggest(int x, int y) { return MAX(x, y); }\n");
lang_walker_smoke!(c_smoke_switch, Language::C, "int f(int x) { switch (x) { case 1: return 1; case 2: return 2; default: return 0; } }\n");

lang_walker_smoke!(cpp_smoke_function, Language::Cpp, "int f(int x) { if (x == 1) { return x; } return 0; }\n");
lang_walker_smoke!(cpp_smoke_class, Language::Cpp, "class A { public: A(int x) : x_(x) {} int get() const { return x_; } private: int x_; };\n");
...
lang_walker_smoke!(cpp_smoke_lambda, Language::Cpp, "int f() { auto add = [](int a, int b) { return a + b; }; return add(1, 2); }\n");
lang_walker_smoke!(cpp_smoke_smart_pointer, Language::Cpp, "void f() { std::unique_ptr<int> p = std::make_unique<int>(42); }\n");
lang_walker_smoke!(cpp_smoke_range_for, Language::Cpp, "void f(std::vector<int> xs) { for (auto x : xs) { use(x); } }\n");

lang_walker_smoke!(swift_smoke_function, Language::Swift, "func f(x: Int) -> Int { if x == 1 { return x }; return 0 }\n");
lang_walker_smoke!(swift_smoke_class, Language::Swift, "class A { var x: Int; init(x: Int) { self.x = x }; func get() -> Int { return x } }\n");
...
lang_walker_smoke!(swift_smoke_guard, Language::Swift, "func f(x: Int?) { guard let y = x else { return }; print(y) }\n");
lang_walker_smoke!(swift_smoke_protocol, Language::Swift, "protocol Drawable { func draw() }\n");
lang_walker_smoke!(swift_smoke_extension, Language::Swift, "extension Int { func doubled() -> Int { return self * 2 } }\n");

lang_walker_smoke!(kotlin_smoke_function, Language::Kotlin, "fun f(x: Int): Int { if (x == 1) { return x }; return 0 }\n");
...
lang_walker_smoke!(kotlin_smoke_when, Language::Kotlin, "fun f(x: Int): Int = when (x) { 1 -> 1; 2 -> 2; else -> 0 }\n");
...
lang_walker_smoke!(ruby_smoke_class, Language::Ruby, "class A\n  def initialize(x)\n    @x = x\n  end\n  def get\n    @x\n  end\nend\n");
...
lang_walker_smoke!(haskell_smoke_data, Language::Haskell, "data Shape = Circle Double | Square Double | Triangle Double Double Double\n");
...
lang_walker_smoke!(php_smoke_function, Language::Php, "<?php function f($x) { if ($x === 1) { return $x; } return 0; }\n");
lang_walker_smoke!(php_smoke_class, Language::Php, "<?php class A { public $x; function __construct($x) { $this->x = $x; } function get() { return $this->x; } }\n");
...
lang_walker_smoke!(d_smoke_class, Language::D, "class A { int x; this(int x) { this.x = x; } int get() { return x; } }\n");
...
lang_walker_smoke!(groovy_smoke_class, Language::Groovy, "class A { int x\n  A(int x) { this.x = x }\n  int get() { return x }\n}\n");

lang_walker_smoke!(objc_smoke_function, Language::ObjectiveC, "int f(int x) { if (x == 1) { return x; } return 0; }\n");
lang_walker_smoke!(objc_smoke_message, Language::ObjectiveC, "void f(NSObject *o) { [o doSomething:42 withFlag:YES]; }\n");

lang_walker_smoke!(tcl_smoke_proc, Language::Tcl, "proc f {x} {\n    if {$x == 1} {\n        return $x\n    }\n    return 0\n}\n");
...
lang_walker_determinism!(python_determinism_function, Language::Python, "def f(x):\n    if x == 1:\n        return x\n    return 0\n");
lang_walker_determinism!(typescript_determinism_function, Language::TypeScript, "function f(x: number): number { if (x === 1) return x; return 0; }\n");
lang_walker_determinism!(javascript_determinism_function, Language::JavaScript, "function f(x) { if (x === 1) return x; return 0; }\n");
lang_walker_determinism!(rust_determinism_function, Language::Rust, "fn f(x: i32) -> i32 { if x == 1 { x } else { 0 } }\n");
lang_walker_determinism!(go_determinism_function, Language::Go, "package p\nfunc f(x int) int { if x == 1 { return x }; return 0 }\n");
lang_walker_determinism!(java_determinism_class, Language::Java, "class A { int f(int x) { if (x == 1) return x; return 0; } }\n");
lang_walker_determinism!(csharp_determinism_class, Language::CSharp, "class A { int F(int x) { if (x == 1) return x; return 0; } }\n");
lang_walker_determinism!(c_determinism_function, Language::C, "int f(int x) { if (x == 1) { return x; } return 0; }\n");
lang_walker_determinism!(cpp_determinism_function, Language::Cpp, "int f(int x) { if (x == 1) { return x; } return 0; }\n");
lang_walker_determinism!(swift_determinism_function, Language::Swift, "func f(x: Int) -> Int { if x == 1 { return x }; return 0 }\n");
lang_walker_determinism!(kotlin_determinism_function, Language::Kotlin, "fun f(x: Int): Int { if (x == 1) { return x }; return 0 }\n");
lang_walker_determinism!(zig_determinism_function, Language::Zig, "fn f(x: i32) i32 { if (x == 1) { return x; } return 0; }\n");
lang_walker_determinism!(ruby_determinism_function, Language::Ruby, "def f(x)\n  if x == 1\n    return x\n  end\n  0\nend\n");
lang_walker_determinism!(haskell_determinism_function, Language::Haskell, "f :: Int -> Int\nf x = if x == 1 then x else 0\n");
lang_walker_determinism!(lua_determinism_function, Language::Lua, "function f(x) if x == 1 then return x end return 0 end\n");
lang_walker_determinism!(php_determinism_function, Language::Php, "<?php function f($x) { if ($x === 1) { return $x; } return 0; }\n");
lang_walker_determinism!(d_determinism_function, Language::D, "int f(int x) { if (x == 1) { return x; } return 0; }\n");
lang_walker_determinism!(groovy_determinism_function, Language::Groovy, "def f(x) { if (x == 1) { return x }; return 0 }\n");
lang_walker_determinism!(r_determinism_function, Language::R, "f <- function(x) { if (x == 1) { return(x) }; return(0) }\n");
lang_walker_determinism!(objc_determinism_function, Language::ObjectiveC, "int f(int x) { if (x == 1) { return x; } return 0; }\n");
lang_walker_determinism!(tcl_determinism_function, Language::Tcl, "proc f {x} {\n    if {$x == 1} {\n        return $x\n    }\n    return 0\n}\n");
lang_walker_determinism!(cobol_determinism_program, Language::Cobol,
    "       IDENTIFICATION DIVISION.\n       PROGRAM-ID. F.\n       PROCEDURE DIVISION.\n           IF X = 1 DISPLAY X END-IF.\n           STOP RUN.\n");

lang_walker_whitespace_stable!(python_ws_function, Language::Python,
    "def f(x):\n    if x == 1:\n        return x\n    return 0\n",
    "\n\ndef  f(x):\n\n    if  x == 1:\n        return x\n\n    return 0\n\n");
lang_walker_whitespace_stable!(typescript_ws_function, Language::TypeScript,
    "function f(x: number): number { if (x === 1) return x; return 0; }\n",
    "function f(x: number): number {\n  if (x === 1) return x;\n  return 0;\n}\n");
lang_walker_whitespace_stable!(rust_ws_function, Language::Rust,
    "fn f(x: i32) -> i32 { if x == 1 { return x; } 0 }\n",
    "fn  f ( x : i32 ) -> i32 {\n    if x == 1 {\n        return x;\n    }\n    0\n}\n");
lang_walker_whitespace_stable!(go_ws_function, Language::Go,
    "package p\nfunc f(x int) int { if x == 1 { return x }; return 0 }\n",
    "package p\n\nfunc f(x int) int {\n\tif x == 1 {\n\t\treturn x\n\t}\n\treturn 0\n}\n");
lang_walker_whitespace_stable!(java_ws_class, Language::Java,
    "class A { int f(int x) { if (x == 1) { return x; } return 0; } }\n",
    "class A {\n  int f(int x) {\n    if (x == 1) {\n      return x;\n    }\n    return 0;\n  }\n}\n");
lang_walker_whitespace_stable!(c_ws_function, Language::C,
