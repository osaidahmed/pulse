...
fn func_finding(name: &str, start_line: u32, end_line: u32) -> Finding {
    Finding {
        ...
        location: Location::Function { name: name.to_string(), start_line, end_line },
        detail: String::new(),
    }
}

fn module_finding() -> Finding {
    Finding { smell: Smell::FileTooLarge, location: Location::Module, detail: String::new() }
}
...
fn finding_strictly_outside_range_is_dropped() {
    let findings = vec![func_finding("before", 5, 10), func_finding("after", 100, 110)];
...
fn module_finding_dropped_when_range_provided() {
    let findings = vec![module_finding(), func_finding("inside", 32, 38)];
    let kept = filter_by_edit_range(findings, Some((30, 40)));
...
fn mixed_findings_only_overlapping_kept() {
    ...
    got.sort();
    let mut expected = vec!["at_boundary_end".to_string(), "at_boundary_start".to_string(), "inside".to_string()];
    expected.sort();
...
fn none_range_keeps_all_function_findings() {
    let findings = vec![func_finding("a", 1, 10), func_finding("b", 100, 200), func_finding("c", 1000, 2000)];
...
fn none_range_drops_module_findings_only() {
    let findings = vec![func_finding("fn1", 1, 10), module_finding(), func_finding("fn2", 50, 60), module_finding()];
    let kept = filter_by_edit_range(findings, None);
    let mut got = names(&kept);
    got.sort();
...
fn range_with_zero_length_keeps_findings_at_that_line() {
    let findings =
        vec![func_finding("at_line_50", 50, 50), func_finding("nearby", 49, 51), func_finding("far_away", 1, 5)];
