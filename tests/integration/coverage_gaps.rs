...
fn invalid_args_prints_usage() {
    let out = Command::new(pulse_bin()).args(["garbage"]).output().unwrap();
...
fn check_unsupported_file_silent() {
    ...
    let out = Command::new(pulse_bin()).args(["check", path.to_str().unwrap()]).output().unwrap();
    assert!(String::from_utf8_lossy(&out.stdout).is_empty());
}

#[test]
fn check_nonexistent_file_silent() {
    let out = Command::new(pulse_bin()).args(["check", "/nonexistent/file.py"]).output().unwrap();
    assert!(String::from_utf8_lossy(&out.stdout).is_empty());
}
...
fn debug_unsupported_file_errors() {
    ...
    let path = dir.path().join("test.xyz");
    std::fs::write(&path, "hello world").unwrap();
    let out = Command::new(pulse_bin()).args(["debug", path.to_str().unwrap()]).output().unwrap();
    assert!(!out.status.success());
}
...
fn analytics_stop_hook_exercises_resolve() {
    ...
    let json = format!(r#"{{"tool_input":{{"file_path":"{}"}}, "session_id":"test-session"}}"#, file.to_str().unwrap());
...
fn baselines_session_dir_without_env() {
    ...
    let json =
        format!(r#"{{"tool_input":{{"file_path":"{}"}}, "session_id":"{}"}}"#, file.to_str().unwrap(), session_id);
...
fn analytics_resolve_skips_malformed_lines() {
    ...
    std::fs::write(baseline_dir.join("manifest.txt"), format!("{}\n", target_file.to_str().unwrap())).unwrap();
    std::fs::write(baseline_dir.join("findings.jsonl"), "this is not json\n{ also not json\n").unwrap();
...
fn analytics_resolve_module_outcome_direct() {
    ...
    std::fs::write(baseline_dir.join("findings.jsonl"), format!("{entry}\n")).unwrap();
