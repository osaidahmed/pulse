...
#[test]
fn audit_idiom_heavy_listcomp_no_findings_after_idf() {
    let (stdout, _stderr, code) = audit_in_dir("idiom_heavy_listcomp");
    assert_eq!(code, 0, "all 8 files have list comp; should be suppressed");
    assert!(stdout.is_empty());
}

#[test]
fn audit_idiom_heavy_dict_get_no_findings_after_idf() {
