...
fn pipeline_run_finds_shotgun_pattern_with_decoys() {
    ...
    assert!(!findings.is_empty(), "expected pattern detection");
}

#[test]
fn pipeline_run_suppresses_when_pattern_dominates_dir() {
    let dir = tempfile::tempdir().unwrap();
    write_python_files(dir.path(), 10, "def f(x):\n    if x == 1:\n        return x\n    return 0\n");
    let findings = run_pipeline(dir.path());
    assert!(findings.is_empty(), "100% saturation suppressed");
}
...
#[test]
fn pipeline_run_orders_findings_by_support_descending() {
    let dir = tempfile::tempdir().unwrap();
...
fn pipeline_findings_have_positive_support() {
    ...
    let findings = run_pipeline(dir.path());
    for f in &findings {
        assert!(f.support >= t().audit.freqt_min_support as u32);
    }
}
...
#[test]
fn pipeline_findings_all_pass_idf_threshold() {
    let dir = tempfile::tempdir().unwrap();
    write_python_files(dir.path(), 5, "def f(x):\n    if x == 1:\n        return x\n    return 0\n");
    write_decoys(dir.path(), 6);
    let findings = run_pipeline(dir.path());
...
fn pipeline_audit_run_returns_audit_finding_with_correct_kind() {
    ...
    let findings = run_pipeline(dir.path());
    for f in &findings {
        assert!(matches!(f.kind, pulse::audit::finding::AuditKind::UncategorizedPattern { .. }));
    }
}
...
fn pipeline_findings_are_within_max_findings_limit() {
    ...
    let findings = run_pipeline(dir.path());
    assert!(findings.len() <= t().audit.max_findings_reported);
}
