...
fn fn_with(name: &str, start: u32, end: u32, structural_hash: u64, skeleton_hash: u64) -> FunctionMetrics {
...
fn exact_duplicates_with_same_hash_emitted_as_finding() {
    let funcs = vec![fn_with("a", 1, 30, 0xABCD, 0x1111), fn_with("b", 50, 79, 0xABCD, 0x2222)];
...
fn distinct_structural_hashes_no_finding() {
    let funcs = vec![fn_with("a", 1, 30, 0xAAA, 0x1), fn_with("b", 50, 79, 0xBBB, 0x2)];
    let findings = detect(&funcs, &t());
...
fn loc_below_threshold_excluded() {
    let funcs = vec![fn_with("a", 1, 2, 0xAAA, 0x1), fn_with("b", 5, 6, 0xAAA, 0x1)];
    let findings = detect(&funcs, &t());
    assert!(findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count() == 0);
}
...
fn skeleton_match_above_higher_loc_floor_emitted() {
    let funcs = vec![fn_with("a", 1, 30, 0xAAA, 0xCAFE), fn_with("b", 50, 79, 0xBBB, 0xCAFE)];
    let findings = detect(&funcs, &t());
    let _ = findings;
}
...
fn skeleton_match_below_skeleton_floor_no_finding() {
    let funcs = vec![fn_with("a", 1, 10, 0xAAA, 0xCAFE), fn_with("b", 50, 59, 0xBBB, 0xCAFE)];
    let findings = detect(&funcs, &t());
...
fn similar_clones_with_loc_size_within_ratio_emitted() {
    let funcs = vec![fn_with("a", 1, 30, 0xAAA, 0xFEED), fn_with("b", 50, 80, 0xBBB, 0xFEED)];
    let _ = detect(&funcs, &t());
}

#[test]
fn similar_clones_with_loc_size_outside_ratio_filtered_out() {
    let funcs = vec![fn_with("a", 1, 25, 0xAAA, 0xFEED), fn_with("b", 50, 200, 0xBBB, 0xFEED)];
    let _ = detect(&funcs, &t());
}

#[test]
fn already_reported_exact_clone_not_duplicated_as_skeleton() {
    let funcs = vec![fn_with("a", 10, 50, 0xCAFE, 0xCAFE), fn_with("b", 60, 100, 0xCAFE, 0xCAFE)];
    let findings = detect(&funcs, &t());
    let dup_count = findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count();
    assert!(dup_count <= 2);
}
...
fn three_or_more_duplicates_emit_single_group_finding() {
    let funcs =
        vec![fn_with("a", 1, 30, 0xCAFE, 0x1), fn_with("b", 50, 79, 0xCAFE, 0x2), fn_with("c", 100, 129, 0xCAFE, 0x3)];
    let findings = detect(&funcs, &t());
...
fn test_function_name_filtered_from_duplication() {
    let funcs = vec![fn_with("test_a", 1, 30, 0xCAFE, 0x1), fn_with("test_b", 50, 79, 0xCAFE, 0x2)];
    let findings = detect(&funcs, &t());
...
fn mixed_test_and_production_function_emits_finding() {
    let funcs = vec![fn_with("test_a", 1, 30, 0xCAFE, 0x1), fn_with("real_b", 50, 79, 0xCAFE, 0x2)];
    let findings = detect(&funcs, &t());
    let dup = findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count();
    assert!(dup >= 1);
}
...
fn dotted_test_function_name_filtered() {
    let funcs = vec![fn_with("module.test_foo", 1, 30, 0xCAFE, 0x1), fn_with("other.test_bar", 50, 79, 0xCAFE, 0x2)];
    let findings = detect(&funcs, &t());
    let dup = findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count();
    assert_eq!(dup, 0);
}
...
fn single_function_no_findings() {
    ...
    let findings = detect(&funcs, &t());
    assert_eq!(findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count(), 0);
}

#[test]
fn duplication_finding_detail_contains_member_names() {
    let funcs = vec![fn_with("alpha", 1, 30, 0xCAFE, 0x1), fn_with("beta", 50, 79, 0xCAFE, 0x2)];
    let findings = detect(&funcs, &t());
...
fn duplication_finding_detail_contains_line_ranges() {
    let funcs = vec![fn_with("alpha", 10, 35, 0xCAFE, 0x1), fn_with("beta", 60, 89, 0xCAFE, 0x2)];
    let findings = detect(&funcs, &t());
    let f = findings.iter().find(|f| f.smell == Smell::CodeDuplication).unwrap();
...
fn raising_min_loc_threshold_suppresses() {
    let funcs = vec![fn_with("a", 1, 30, 0xCAFE, 0x1), fn_with("b", 50, 79, 0xCAFE, 0x2)];
    let mut th = t();
...
fn similar_clone_with_loc_at_size_ratio_boundary() {
    let funcs = vec![fn_with("a", 1, 30, 0xAAA, 0xFEED), fn_with("b", 50, 87, 0xBBB, 0xFEED)];
    let _ = detect(&funcs, &t());
}

#[test]
fn similar_clone_with_loc_just_outside_ratio() {
    let funcs = vec![fn_with("a", 1, 30, 0xAAA, 0xFEED), fn_with("b", 50, 110, 0xBBB, 0xFEED)];
    let _ = detect(&funcs, &t());
}
...
fn mixed_loc_in_group_some_below_floor() {
    let funcs =
        vec![fn_with("a", 1, 30, 0xCAFE, 0x1), fn_with("b", 50, 51, 0xCAFE, 0x2), fn_with("c", 100, 129, 0xCAFE, 0x3)];
    let findings = detect(&funcs, &t());
    let _ = findings;
}
...
fn group_with_all_below_threshold_yields_no_finding() {
    let funcs = vec![fn_with("a", 1, 2, 0xCAFE, 0x1), fn_with("b", 5, 6, 0xCAFE, 0x2)];
    let findings = detect(&funcs, &t());
    assert_eq!(findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count(), 0);
}
...
fn skeleton_finding_does_not_duplicate_already_reported_exact() {
    let funcs = vec![fn_with("a", 10, 50, 0xCAFE, 0xDEAD_BEEF), fn_with("b", 60, 100, 0xCAFE, 0xDEAD_BEEF)];
    let findings = detect(&funcs, &t());
    let dup = findings.iter().filter(|f| f.smell == Smell::CodeDuplication).count();
    assert!(dup <= 2);
}
