...
pub fn plain_ctx(suppression: &pulse_audit::suppression::AuditSuppression) -> pulse_audit::output::RenderCtx<'_> {
    pulse_audit::output::RenderCtx { root: None, show_noise: false, suppression, cpg_enabled: false }
}

pub fn audit_fixtures_root() -> PathBuf {
    pulse_testkit::workspace_root().join("tests").join("fixtures").join("audit")
}
