...
fn audit_thresholds_default_round_trips_via_const() {
    ...
    assert!(
        (direct.package_metrics.martin_distance_warning - via_default.package_metrics.martin_distance_warning).abs()
            < 1e-12
    );
    assert!(
        (direct.package_metrics.martin_distance_alert - via_default.package_metrics.martin_distance_alert).abs()
            < 1e-12
    );
...
fn audit_thresholds_is_copy() {
    ...
    let copy = original;
    assert!(
        (original.package_metrics.martin_distance_warning - copy.package_metrics.martin_distance_warning).abs() < 1e-12
    );
}
