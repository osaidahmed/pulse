...
const PHP: TaintLang = TaintLang {
    ...
    sources: &[],
    prop_sources: &["$_GET", "$_POST", "$_REQUEST", "$_COOKIE", "$_FILES"],
...
const KOTLIN: TaintLang = TaintLang {
    ...
    aug_kinds: &[],
    call_kinds: &["call_expression"],
    sources: &["getParameter", "getHeader", "getQueryParameter", "getInputStream", "getReader", "readLine", "getenv"],
    prop_sources: &[],
    source_kinds: &[],
    ident_kinds: &["identifier"],
