...
use crate::finding::{AuditFinding, AuditKind, ImportConfidence, NaturalnessEvidence};
use pulse_syntax::parse::Language;
use pulse_syntax::walk::{find_child_by_kind, node_text, DepthGuard};
use pulse_thresholds::{AuditThresholds, NaturalnessThresholds};
...
pub fn run(typed_files: &[(PathBuf, Language)], thresholds: &AuditThresholds) -> Vec<AuditFinding> {
    run_from(&crate::corpus::Corpus::load(typed_files), thresholds)
}

pub fn run_from(corpus: &crate::corpus::Corpus, thresholds: &AuditThresholds) -> Vec<AuditFinding> {
    let mut out = Vec::new();
...
fn fn_kinds_for(lang: Language) -> Option<&'static [&'static str]> {
    let kinds = pulse_syntax::langkinds::function_kinds(lang);
