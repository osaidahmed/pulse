...
const SWAP_CONFIG: SwapConfig =
    SwapConfig { samples: 200, step_multiplier: 5, seed: 0x5EED_5EED_5EED_5EED, max_cols: 256 };
...
pub fn build_findings(clusters: Vec<RawCluster>, ctx: &ScoringCtx) -> Vec<AuditFinding> {
    let mut findings: Vec<AuditFinding> =
        clusters.into_iter().filter_map(|c| build_categorized_finding(c, ctx)).collect();
...
fn cluster_is_corpus_idiom(cluster: &RawCluster, ctx: &ScoringCtx) -> bool {
    let Some(lang) = cluster.locations.first().and_then(|(f, _)| pulse_syntax::parse::detect_language(f)) else {
        return false;
    };
...
fn finding_from_cluster(cluster: RawCluster, score: Option<f64>, category: PatternCategory) -> AuditFinding {
    ...
    let locations = cluster.locations.into_iter().map(|(file, line)| AuditLocation { file, line }).collect();
    AuditFinding {
...
pub fn apply_idf(clusters: Vec<RawCluster>, total_files: usize, thresholds: &AuditThresholds) -> Vec<AuditFinding> {
    if total_files == 0 {
        return Vec::new();
    }
...
fn legacy_order(a: &AuditFinding, b: &AuditFinding) -> std::cmp::Ordering {
    b.support.cmp(&a.support).then(b.file_count.cmp(&a.file_count)).then(fingerprint_of(a).cmp(&fingerprint_of(b)))
}
