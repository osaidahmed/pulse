...
use crate::finding::{CycleMembership, MartinMetrics, MartinTier};
use crate::output::helpers::{confidence_str, display_path, write_capped_list, ListLayout};
...
pub fn write_cycle(
    ...
    let _ = writeln!(
        ...
        c.members.len(),
        crate::cycle_shapes::shape_label(c.shape)
    );
...
pub fn cycle_json(c: &CycleMembership, root: Option<&Path>) -> serde_json::Value {
    ...
    serde_json::json!({
        "kind": "ImportCycle",
        "size": c.members.len(),
        "shape": crate::cycle_shapes::shape_label(c.shape),
        "members": members,
        "edges": edges,
