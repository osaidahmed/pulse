use pulse_audit::corpus_df;
use pulse_audit::scoring::corpus_idiom_suppressed;
...
fn corpus_idiom_suppression_is_off_by_default_and_config_opts_in() {
    let base = pulse_thresholds::Thresholds::default();
    ...
    std::fs::write(dir.path().join(".pulse.toml"), "[thresholds.pattern_mining]\ncorpus_idiom_frequency = 0.1\n")
        .unwrap();
    let cfg = pulse_config::load_config(dir.path()).expect("config loads");
    let resolved = pulse_config::resolve_thresholds(Some(&cfg), Language::Rust);
