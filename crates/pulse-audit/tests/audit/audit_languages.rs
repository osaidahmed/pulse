use pulse_audit::walker::extract_subtrees;
...
fn extract(lang: Language, src: &str) -> Vec<pulse_audit::walker::SubtreeRecord> {
...
fn lang_kinds_table_has_22_entries() {
    use pulse_audit::lang_kinds::skippable_root_kinds;
...
fn is_skippable_root_returns_false_for_unknown_kind() {
    use pulse_audit::lang_kinds::is_skippable_root;
    for (lang, _, _) in LANGS {
