...
use pulse_audit::vuln_deps;
use pulse_buildmeta::registry::{
    parse_package, parse_version_detail, FreshnessThresholds, PackageInfo, VersionInfo, VersionKey,
};
...
fn progress_only_for_many_deps_on_a_terminal() {
    use pulse_audit::freshness::should_show_progress;
...
fn parallel_lookups_are_deterministic_over_many_cached_deps() {
    ...
    let names = |fs: &[pulse_audit::finding::AuditFinding]| -> Vec<String> {
        fs.iter()
            .filter_map(|f| match &f.kind {
...
fn cache_write_round_trips_and_overwrites() {
    use pulse_buildmeta::registry::write_atomic;
    let dir = tempfile::tempdir().unwrap();
...
fn vuln_evidence(findings: &[pulse_audit::finding::AuditFinding]) -> Vec<&pulse_audit::finding::VulnDepEvidence> {
    findings
        .iter()
        .filter_map(|f| match &f.kind {
