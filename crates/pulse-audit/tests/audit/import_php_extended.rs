...
use pulse_audit::imports::{extract_imports, resolve_target};
use pulse_syntax::parse::{parse_only, Language};

fn extract(source: &str) -> Vec<pulse_audit::imports::RawImport> {
...
fn path_suffix_for_simple_namespace() {
    let s = pulse_audit::imports::resolve_by_suffix("Foo\\Bar", Language::Php, &std::collections::HashSet::new());
    assert!(s.is_none());
}
