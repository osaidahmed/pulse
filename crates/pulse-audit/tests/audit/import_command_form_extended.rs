...
fn extract(source: &str, lang: Language) -> Vec<pulse_audit::imports::RawImport> {
...
fn resolve_target_for_tcl_uses_command_form_candidates() {
    ...
    let _ = pulse_audit::imports::resolve_target(raw, source_file, project_root, Language::Tcl);
}
...
fn resolve_target_for_cobol_uses_command_form_candidates() {
    ...
    let project_root = Path::new("/tmp/proj");
    let _ = pulse_audit::imports::resolve_target(raw, source_file, project_root, Language::Cobol);
}
