...
use pulse_audit::{corpus_df, record_extraction, walk_typed_source_files_filtered, IgnoreFilter};
...
fn drive_sweep(corpus_root: &Path) {
    ...
    let snapshot_dir = pulse_testkit::workspace_root().join("target/corpus-df-snapshots");
