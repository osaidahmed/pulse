...
use pulse_audit::class_registry::{
...
fn class_atfd_dedupes_distinct_pairs() {
    ...
    let foreign_for = |m: pulse_audit::call_graph::MethodIndex| -> Vec<(String, String)> {
...
fn class_tcc_three_methods_all_share_field() {
    ...
    let (reg, graph) = build_registry(defs.clone());
    let fields_for = |m: pulse_audit::call_graph::MethodIndex| -> (Vec<String>, bool) {
        let id = graph.registry.get(m).unwrap();
