...
use pulse_audit::component_thresholds::{is_owned_by, is_root_owned, owning_dir, primary_file, Stratum};
use pulse_audit::corpus::Corpus;
use pulse_audit::definitions::{definitions_for_file, definitions_from};
use pulse_audit::duplication_clusters::{cluster_members, run_from as clones_run_from};
...
use pulse_audit::vuln_clones::run_from as vuln_run_from;
use pulse_audit::{run_with_filter_online, AuditOpts, IgnoreFilter, PassChoice, RunCtx};
...
fn extract(src: &str, lang: Language) -> Vec<pulse_audit::calls::RawCall> {
    let tree = pulse_syntax::parse::parse_only(src, lang).expect("parse must succeed");
    pulse_audit::calls::extract_calls(&tree, src, lang)
}
